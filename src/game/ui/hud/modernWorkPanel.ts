 ...
 */
export function getWorkPanelLayout(canvas: HTMLCanvasElement): {
  x: number;
  y: number;
  width: number;
  height: number;
} {
  const hotbarHeight = canvas.height * 0.06; // Hotbar is 6% of screen
  ...
  const gap = canvas.height * 0.01; // 1% gap
...
export function drawWorkPanelContainer(
  ...
  const layout = getWorkPanelLayout(canvas);
