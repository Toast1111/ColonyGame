...
export function drawPlacementUI(game: any) {
  ...
  ctx.fillStyle = can ? 'rgba(75, 159, 255, 0.6)' : 'rgba(255, 107, 107, 0.6)'; ctx.fillRect(scr.x, scr.y, sw, sh);
  ...
  const drawBtn = (r: any, label: string, disabled = false) => { 
    ...
    ctx.fillStyle = disabled ? 'rgba(219, 234, 254, 0.5)' : '#dbeafe'; ctx.font = game.getScaledFont(18, '600'); ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
