...
export function updateColonistFSM(game: any, c: Colonist, dt: number) {
  ...
  function evaluateIntent(): { state: ColonistState; prio: number; reason: string } | null {
    ...
    // Medical bed rest intent: only trigger for meaningful injury or missing HP
    const injuries = c.health?.injuries?.length || 0;
    const missingHp = Math.max(0, 100 - (c.hp || 100));
    const needsBedRest = injuries > 0 || missingHp >= 12;
    if (needsBedRest && !c.isDrafted && !(starving && !waitingForDoctor)) {
    ...
    if (hungry && hasEdible)
      set('eat', 70 + Math.min(20, hunger - 75), 'hunger');
  ...
  switch (c.state) {
    ...
    case 'beingTreated': {
      ...
      const injuries = c.health?.injuries?.length || 0;
      const missingHp = Math.max(0, 100 - (c.hp || 100));
      const waitingForDoctor = !!isBeingTreated && !!doctorId;
      ...
      if (!waitingForDoctor && injuries === 0 && missingHp < 6) {
        (c as any).isBeingTreated = false;
        (c as any).doctorId = undefined;
        changeState('seekTask', 'no longer needs treatment');
        break;
      }

      if (!waitingForDoctor && hunger > 85) {
        if (game.leaveBuilding) game.leaveBuilding(c);
        changeState('eat', 'hungry while waiting for treatment');
        break;
      }
  ...
  case 'eat': {
    ...
    case 'build': {
      updateBuildState(c, game, dt, changeState);
      break;
    }
    case 'harvest': {
